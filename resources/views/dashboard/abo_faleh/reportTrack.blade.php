...
@section('content')
        ...
        <div class="modern-header">
            <div class="header-content">
                ...
                <div class="header-actions">
                    ...
                    <button class="modern-btn btn-excel" onclick="exportToExcel()">
                        <i data-lucide="file-spreadsheet"></i>
                        تصدير Excel
                    </button>
                    <button class="modern-btn btn-pdf" onclick="exportToPDF()">
                        <i data-lucide="file-text"></i>
                        تصدير PDF
                    </button>
                    <a href="{{ route('ABO_FALEH.storehouseReport') }}" class="modern-btn nav-tab">
                ...
                foreach ($grouped as $rows) {
                    foreach ($rows as $row) {
                        if (!empty($row['warehouse_items'])) {
                            foreach ($row['warehouse_items'] as $wItem) {
                                if ($wItem['quantity'] < 0) {
                                    $productName = $wItem['product_name'];
                                    $quantity = abs($wItem['quantity']);
                                    if (!isset($total_products_sold[$productName])) {
                                        $total_products_sold[$productName] = 0;
                                    }
                                    $total_products_sold[$productName] += $quantity;
                                }
                            }
                        }
                    }
            ...
            <div class="pagination-container">
                <div class="pagination-left">
                    <div class="pagination-info">
                        عرض <span id="currentStart">1</span> - <span id="currentEnd">50</span> من <span
                            id="totalRecords">0</span> سجل
                    </div>
                        ...
                        <tbody id="tableBody">
                            @foreach ($grouped as $rows)
                                @php
                                    ...
                                    $warehouseInfo = [];
                                    foreach ($rows as $r) {
                                        if (!empty($r['warehouse_items'])) {
                                            foreach ($r['warehouse_items'] as $wItem) {
                                                $warehouseInfo[] = $wItem;
                                            }
                                        }
                                    }
                                @endphp
                                <tr class="data-row" data-type="{{ $types }}" data-group="{{ $row['group'] }}"
                                    data-client="{{ $row['client'] }}" data-arrival="{{ $arrival }}"
                                    data-departure="{{ $departure }}" data-duration="{{ $duration }}"
                                    data-duration-minutes="{{ $durationMinutes }}" data-date="{{ $row['date'] }}"
                                    data-receipt="{{ $receipt }}" data-payment="{{ $payment }}"
                                    data-invoice="{{ $invoice }}" data-expense="{{ $expense }}"
                                    data-credit="{{ $credit_note }}" data-note="{{ $repNote }}"
                                    data-typeclass="{{ $typeClass }}" style="display: none;">
                                    ...
                                    <td>
                                        @if (!empty($warehouseInfo))
                                            @foreach ($warehouseInfo as $wItem)
                                                <div class="warehouse-info">
                                                    <strong>{{ $wItem['warehouse_name'] }}</strong>
                                                    <br>
                                                    <small>{{ $wItem['product_name'] }}</small>
                                                    <br>
                                                    <span class="quantity-badge {{ $wItem['quantity'] > 0 ? 'positive' : 'negative' }}">
                                                        {{ $wItem['quantity'] > 0 ? '+' : '' }}{{ $wItem['quantity'] }}
                                                    </span>
                                                </div>
                                            @endforeach
                                        @else
                                            ...
                                            @foreach ($warehouseInfo as $wItem)
                                                <div class="balance-display">
                                                    <div class="balance-row">
                                                        <span class="balance-label">قبل:</span>
                                                        <span class="balance-value before">{{ number_format($wItem['balance_before'], 0) }}</span>
                                                    </div>
                                                    <div class="balance-arrow">
                                                        <i data-lucide="arrow-down"></i>
                                                    </div>
                                                    <div class="balance-row">
                                                        <span class="balance-label">بعد:</span>
                                                        <span class="balance-value after">{{ number_format($wItem['balance_after'], 0) }}</span>
                                                    </div>
                                                </div>
                                            @endforeach
                                        @else
    ...
    <script>
        ...
        function exportToExcel() {
            const rows = Array.from(document.querySelectorAll('.data-row')).filter(row => row.style.display !== 'none');
            const data = [];

            data.push(['النوع', 'المجموعة', 'العميل', 'الوصول', 'الانصراف', 'المدة', 'التاريخ', 'سند قبض', 'المدفوع', 'الفاتورة', 'سند صرف', 'مرتجع', 'ملاحظات']);
            ...
            const totalInvoice = document.getElementById('totalInvoice').textContent;
            ...
            data.push(['', '', '', '', 'الإجمالي', totalMinutes, '', totalReceipt, totalPayment, totalInvoice, totalExpense, totalCredit, '']);

            const ws = XLSX.utils.aoa_to_sheet(data);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "تقرير الموظف");

            const fileName = `تقرير_الموظف_${new Date().toISOString().split('T')[0]}.xlsx`;
        ...
        function exportToPDF() {
            ...
            doc.setFontSize(18);
            doc.text('التقرير الموحد للموظف', doc.internal.pageSize.width / 2, 15, { align: 'center' });

            doc.setFontSize(12);
            doc.text('{{ $user->name }}', doc.internal.pageSize.width / 2, 25, { align: 'center' });
            ...
            doc.autoTable({
                head: [['النوع', 'المجموعة', 'العميل', 'الوصول', 'الانصراف', 'المدة', 'التاريخ', 'سند قبض', 'المدفوع', 'الفاتورة', 'سند صرف', 'مرتجع', 'ملاحظات']],
                body: tableData,
                startY: 35,
                styles: {
                    font: 'helvetica',
            ...
            const fileName = `تقرير_الموظف_${new Date().toISOString().split('T')[0]}.pdf`;
            doc.save(fileName);
        }
