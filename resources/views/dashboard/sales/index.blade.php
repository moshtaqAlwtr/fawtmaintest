...
@php
    ...
    $displayBranch = $displayBranchId ? App\Models\Branch::find($displayBranchId) : null;
    $isAllBranches = $displayBranchId === '0' || (auth()->user()->role === 'main' && !$displayBranchId);
@endphp
        ...
        <div class="d-flex align-items-center justify-content-between flex-wrap">
            <div>
                <strong style="color: #1a202c; font-size: 1.05rem;">
                    <i class="feather icon-info" style="color: #667eea;"></i>
                    الفرع المعروض:
                </strong>
                <span style="color: #667eea; font-weight: 700; font-size: 1.1rem;">
                    @if ($isAllBranches)
                        🌍 جميع الفروع
                    @elseif ($displayBranch)
                        🏢 {{ $displayBranch->name }}
                    @elseif (auth()->user()->branch_id)
                        🏢 {{ auth()->user()->currentBranch->name ?? 'غير محدد' }}
                    @else
                        غير محدد
                    @endif
                </span>
