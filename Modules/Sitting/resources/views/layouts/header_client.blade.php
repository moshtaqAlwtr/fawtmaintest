...
$(document).ready(function() {
    ...
    // ========================================
    // جلب الإشعارات - ✅ زر الرد يظهر للجميع الآن
    // ========================================
                ...
                if (response.success) {
                    ...
                    if (count > 0) {
                        notifications.forEach(notification => {
                            ...
                            // ✅ زر الرد - يظهر للجميع الآن (تم إزالة الشرط)
                            let replyButton = `
                        ...
                        notificationList.html('<p class="text-center p-2 text-muted">لا يوجد إشعارات جديدة</p>');
                    }
                } else {
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $(`.notification-item[data-notification-id="${notificationId}"]`).fadeOut(300, function() {
                        ...
                        // إذا لا يوجد إشعارات
                        if (newCount === 0) {
                            $('#notification-list').html('<p class="text-center p-2 text-muted">لا يوجد إشعارات جديدة</p>');
                        }
                    });
                } else {
                    if (typeof toastr !== 'undefined') {
